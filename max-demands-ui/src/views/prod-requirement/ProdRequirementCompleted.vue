<template>
  <div>
    <div class="page-header">
      <h1 class="page-header-title">已完成产品需求</h1>
      <p class="page-header-desc">查看和管理已完成状态的产品子需求</p>
    </div>
    ...
    <el-dialog v-model="dialogVisible" title="编辑产品需求" width="760px">
...
const dialogBizReqList = computed(() => {
  return bizReqList.value
})

const { historyMap, loadHistory, saveHistory } = useInputHistory('prod-requirement-completed', {
  bizReqId: {
    limit: 3,
    getLabel: (value, form) => {
...
const COLUMN_VISIBLE_KEY = 'prod-requirement-completed-visible-columns'
const columnOptions = [
...
const { colWidths, loadColWidths, handleHeaderDragend } = useColumnWidth(
  'prod-requirement-completed-col-widths',
  {
    prodReqCode: 280,
    bizReqName: 150,
    systemName: 120,
...
const fetchList = async () => {
  loading.value = true
  try {
    const params = { ...page.value, ...query.value, completedOnly: true }
